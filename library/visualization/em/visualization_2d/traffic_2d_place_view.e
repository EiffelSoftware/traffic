...
class
	TRAFFIC_2D_PLACE_VIEW

inherit

	TRAFFIC_2D_VIEW [TRAFFIC_PLACE]
		undefine
			default_create,
			out,
			set_x_y
...
feature -- Initialization
	...
	make (a_item: like item) is
		...
		do
			...
			make_from_coordinates ( item.position.x - (item.width/2).max (5), item.position.y - (item.breadth/2).max (5),
									item.position.x + (item.width/2).max (5), item.position.y + (item.breadth/2).max (5))

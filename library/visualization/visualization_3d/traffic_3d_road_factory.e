...
feature -- Decision process
	...
	add_road_type (procedure: PROCEDURE[ANY,TUPLE]; key: STRING) is
		...
		require
			...
			key_valid: key /= void and then not key.is_empty
		do
			road_templates.force (procedure, key)
		ensure
			road_templates.item (key) = procedure
		end
	...
	remove_road_type (key: STRING) is
		...
		require
			key_valid: key /= void and then not key.is_empty
		do
			road_templates.remove (key)
		ensure
			road_templates.removed	
		end
...
feature {NONE} -- Implementation
	...
	gaugers: HASH_TABLE[FUNCTION [ANY, TUPLE, STRING], STRING]
			-- Helps to decide the kind of road to draw.
			
	road_templates: HASH_TABLE[PROCEDURE[ANY,TUPLE], STRING]
			-- Containter of all types of roads.
...
feature {EM_3D_OBJECT_FACTORY} -- Deferred features that should not be accessible from the outside
	...
	specify_object is
		...
		do
			road_templates.item(decision).apply
		end		
